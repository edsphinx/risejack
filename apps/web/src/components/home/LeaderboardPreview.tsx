...
export function LeaderboardPreview() {
    ...
    const fetchLeaders = useCallback(async () => {
        try {
            ...
            const newDataHash = JSON.stringify(newEntries.map(e => ({
                w: e.walletAddress,
                v: e.value,
                r: e.rank
            })));
                ...
                <div className="leaderboard-list">
                    {leaders.map((leader, idx) => (
                        <div
                            ...
                            className={`leaderboard-row rank-${idx + 1}`}
                        >
                            <span className="rank">{getMedal(idx + 1)}</span>
                            <span className="player-info">
                                <span className="display-name">{leader.displayName || truncateAddress(leader.walletAddress)}</span>
                                <span className="level">#{leader.rank}</span>
                            </span>
