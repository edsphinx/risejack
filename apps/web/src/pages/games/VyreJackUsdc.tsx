  ...
  useEffect(() => {
    const checkAndApprove = async () => {
      ...
      try {
        ...
        logger.log('[VyreJackUsdc] Current allowance:', allowance.amount.toString());
        ...
        if (!sessionKey) {
          logger.log('[VyreJackUsdc] No session key, showing approval modal');
        ...
        logger.log('[VyreJackUsdc] Auto-approving via session key...');
        ...
        try {
            ...
            params: [
              {
                calls: [
                  {
                    to: USDC_TOKEN_ADDRESS.toLowerCase() as `0x${string}`,
                    value: '0x0',
                    data,
                  },
                ],
          ...
          logger.log('[VyreJackUsdc] Auto-approval successful!');
          ...
          logger.warn('[VyreJackUsdc] Session key approval failed:', sessionError.message);
        ...
        <div className="fixed inset-0 z-40 bg-black/80 backdrop-blur-sm flex items-center justify-center">
          <div className="text-center p-8">
            ...
            <p className="text-white text-lg">
              {isAutoApproving ? 'Auto-approving USDC...' : 'Checking permissions...'}
            </p>
          </div>
        </div>
      )}
