# RISECASINO Documentation

## 📁 Structure

```
docs/
├── whitepaper/
│   └── WHITEPAPER.md          # Public-facing vision document
├── tokenomics/
│   ├── CHIP_TOKEN.md          # Token design and economics
│   ├── STAKING_MODEL.md       # Staking mechanics and yields
│   └── LIQUIDITY_POOLS.md     # AMM/DEX design
├── architecture/
│   ├── SMART_CONTRACTS.md     # Contract architecture
│   ├── BACKEND_EVALUATION.md  # Backend stack decisions
│   ├── FRONTEND.md            # Frontend architecture
│   └── CARD_ENGINE.md         # Multi-platform card rendering (Planned)
├── roadmap/
│   ├── ROADMAP.md             # Phases, milestones, current status
│   ├── CHANGELOG.md           # Version history and release notes
│   └── PLANNED_FEATURES.md    # Detailed feature specifications
├── growth/
│   ├── GROWTH_STRATEGY.md     # Master growth & monetization plan
│   └── DATA_SCHEMA.md         # SQL schema for analytics platform
├── business/
│   ├── REVENUE_MODEL.md       # All revenue streams
│   └── RISK_ANALYSIS.md       # Economic risks and mitigations
└── games/
    └── RISEJACK.md            # Blackjack game specification
```

## 📋 Documents Overview

| Document            | Purpose                                         | Audience             |
| ------------------- | ----------------------------------------------- | -------------------- |
| **Whitepaper**      | Vision, value proposition, high-level overview  | Investors, Community |
| **Tokenomics**      | Token distribution, emissions, utility          | Analysts, Investors  |
| **Architecture**    | Technical design and implementation             | Developers           |
| **Roadmap**         | Milestones, progress tracking, planned features | Team, Everyone       |
| **Growth Strategy** | Viral loops, data monetization, AI roadmap      | Team, Investors      |
| **Business**        | Revenue model, sustainability analysis          | Team, Investors      |
| **Games**           | Individual game specifications                  | Developers, Auditors |

## 🚀 Reading Order

1. [Whitepaper](./whitepaper/WHITEPAPER.md) (vision)
2. [Roadmap](./roadmap/ROADMAP.md) (where we're going)
3. [Growth Strategy](./growth/GROWTH_STRATEGY.md) (how we grow)
4. [Revenue Model](./business/REVENUE_MODEL.md) (how we make money)
5. [Data Schema](./growth/DATA_SCHEMA.md) (what we capture)
6. [CHIP Token](./tokenomics/CHIP_TOKEN.md) (token design)
7. [Card Engine](./architecture/CARD_ENGINE.md) (multi-platform rendering)
8. Individual game specs

## 📈 Progress Tracking

- **Current Status**: See [ROADMAP.md](./roadmap/ROADMAP.md)
- **Release History**: See [CHANGELOG.md](./roadmap/CHANGELOG.md)
- **Upcoming Features**: See [PLANNED_FEATURES.md](./roadmap/PLANNED_FEATURES.md)
- **Casino RPG Vision**: See [MINIMAL_RPG_PLAN.md](./roadmap/MINIMAL_RPG_PLAN.md)
