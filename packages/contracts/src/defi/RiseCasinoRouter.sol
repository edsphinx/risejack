...
library RiseCasinoLibrary {
    ...
    function getReserves(
        ...
        (uint112 reserve0, uint112 reserve1,) =
            RiseCasinoPair(pairFor(factory, tokenA, tokenB)).getReserves();
...
contract RiseCasinoRouter {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) =
            RiseCasinoLibrary.getReserves(factory, tokenA, tokenB);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to =
                i < path.length - 2 ? RiseCasinoLibrary.pairFor(factory, output, path[i + 2]) : _to;
            RiseCasinoPair(RiseCasinoLibrary.pairFor(factory, input, output))
    ...
    function swapExactTokensForTokens(
        ...
        require(
            ...
            "RiseCasinoRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        IERC20(path[0])
            .safeTransferFrom(
                msg.sender, RiseCasinoLibrary.pairFor(factory, path[0], path[1]), amounts[0]
            );
    ...
    function swapExactETHForTokens(
        ...
        assert(
            IWETH(WETH).transfer(RiseCasinoLibrary.pairFor(factory, path[0], path[1]), amounts[0])
        );
        _swap(amounts, path, to);
    }
