...
contract VyreJackETH is IVRFConsumer, ReentrancyGuard {
    ...
    /// @notice House fee in basis points (200 = 2%)
    uint256 public constant HOUSE_FEE_BPS = 200;
    ...
    /// @dev Configurable for Rise Chain's fast 10ms blocks (default: 30 seconds)
    uint256 public vrfTimeout = 30 seconds;

    /// @notice Cooldown between games per player (rate limiting)
    /// @dev Configurable (default: 5 seconds for fast chains)
    ...
    event VRFTimeoutChanged(uint256 newTimeout);
    event GameCooldownChanged(uint256 newCooldown);
        ...
        checkCooldown(msg.sender)
    {
        ...
        uint256 netBet = msg.value - houseFee;

        // Track exposure (max possible payout) based on net bet
     ...
     * @dev Anyone can call this after vrfTimeout has passed
     */
    ...
    /**
     * @notice Set VRF request timeout
     * @param _timeout New timeout in seconds (min: 10s, max: 10 minutes)
     */
    function setVRFTimeout(
        ...
        vrfTimeout = _timeout;
        emit VRFTimeoutChanged(_timeout);
    }

    /**
     * @notice Set game cooldown between plays
     * @param _cooldown New cooldown in seconds (max: 60 seconds)
     */
    function setGameCooldown(
        ...
        emit GameCooldownChanged(_cooldown);
    }
